...
const BusquedaAños = ({ response, id }: any) => {
  const [año, setAño]: any = useState(null);
    ...
    if (data.length >= 1) {
      return (
        <div className="container flex flex-wrap flex-row justify-center mx-auto mb-3 gap-4">
          {data?.map((item: any) => (
            <>
              <div className="pb-4 w-[16rem] h-auto">
            ...
            <div className="mb-3 sm:w-full">
              ...
              >
                ...
                {response.map((item: any) => (
                  <option key={item.contador} value={item.year}>
                    {item.year}
                  </option>
                ))}
              </select>
...
//         />
//       </svg>
//       <p className="text-center">{item.pdf_nom}</p>
//     </a>
//   </>
// ));
// }
